...
export class SymbolDetector {
  ...
  /** Payline wins are supplied by the server; client does not compute line masks here. */
  public getWins(_data: Data): Wins {
    return new Wins(new Map());
  }

  public getGrids(lineMask: number[][], data: Data): Grid[] {
    const grids: Grid[] = [];
    for (let x = 0; x < lineMask[0].length; x++) {
      for (let y = 0; y < lineMask.length; y++) {
        if (lineMask[y][x] !== 1) {
          continue;
        }
