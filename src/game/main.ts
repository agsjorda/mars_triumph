...
const StartGame = (parent: string) => {
	...
	/** Dev console: open a dialog. Example: showDialog({ type: 'BigWin', winAmount: 50000 }) */
	(window as any).showDialog = (params: {
		...
		Object.keys(params).forEach((k) => {
			if (!['type', 'winAmount', 'freeSpins', 'betAmount', 'isRetrigger'].includes(k)) {
				(config as any)[k] = (params as any)[k];
			}
		});
		dialogs.showDialog(scene, config as any);
	};
	const appElement = document.getElementById('root');
	if (appElement) {
