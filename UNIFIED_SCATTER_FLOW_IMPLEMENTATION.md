...
2. **Unified sequence**:
   ...
   - Merge / gather scatter symbols to center
...
- **`runScatterFlow(options)`**: Main unified flow orchestrator
  - Calls symbols' `animateScatterSymbols()` to merge/gather
...
- **Consistent timing**: merge → win hold → dialog → idle → unmerge
...
1. Normal scatter trigger in base game
2. Scatter retrigger during bonus
...
- ✅ Idle transition
- ✅ Unmerge on dialog close
- ✅ No separate retrigger hacks — all flows use same pipeline
